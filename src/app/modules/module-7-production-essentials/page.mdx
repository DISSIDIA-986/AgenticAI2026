...
Agent 真正难的不是“跑起来”，而是“出了问题时你能不能看见、拦住、接管、回滚”。这一章把 Agent 当成生产系统来对待，而不是当成一段会输出文本的 demo。

## 读这一章时，先盯住三件事

- Guardrails 为什么必须分成输入、工具调用、输出三层
- 哪些决策必须进 human-in-the-loop，而不是继续自动化
- Agent 的观测、评估和成本控制为什么必须像线上服务一样严格

## 学完后你应该能做什么

- 画出一个 Agent 的最小生产防线，而不是只谈 prompt 和模型
- 给高风险操作设计审批门、降级路径和熔断策略
- 建立一套能持续改进 Agent 的评估和反馈闭环
...
<Image
  src="/course-visuals/module-7/01-guardrail-stack-input-to-output.svg"
  alt="Guardrail 分层防线图，展示输入、工具调用、输出和系统控制四层如何沿着 Agent 运行路径逐层防护。"
  width={1600}
  height={900}
/>
...
<Image
  src="/course-visuals/module-7/02-hitl-trigger-matrix.png"
  alt="Human-in-the-Loop 触发矩阵，使用置信度与影响两个维度区分全自动、事后审计、自动重试和强制人工审批。"
  width={1920}
  height={1080}
/>
...
<Image
  src="/course-visuals/module-7/03-observability-telemetry-pipeline.png"
  alt="Agent 可观测性遥测管线图，展示运行时事件如何进入 Trace、Metrics、Logs、Eval 和 Alert 系统，并反馈给值班工程师。"
  width={1920}
  height={1080}
/>
...
- [ ] 输入、工具调用、输出三层 guardrail 已经分开设计，而不是混成一个大过滤器
- [ ] 高影响低置信度操作默认进入 human-in-the-loop，而不是继续自动执行
- [ ] trace、tool calls、guardrail violations、cost 四类信号都可观测
- [ ] prompt 变更、模型切换、权限放开都支持评估、灰度和快速回滚
