...
Agent 真正有价值，不是因为它会“说”，而是因为它会“做”。这一章要回答的是：怎么把外部能力安全地接给 Agent，以及为什么 MCP 会成为这一层的标准接口。

## 读这一章时，先盯住三件事

- tool schema 为什么不是注释，而是模型决策的输入
- MCP 到底解决了哪种工具生态碎片化问题
- 为什么工具安全的关键不在 LLM，而在运行时校验和权限边界

## 学完后你应该能做什么

- 设计一个 LLM 真能正确使用的 tool schema
- 向别人解释 MCP 的 Host、Client、Server、Transport 各自扮演什么角色
- 把原子 tool 和高层 skill 清楚分层，避免工具层职责膨胀
...
<Image
  src="/course-visuals/module-3/01-tool-calling-lifecycle.svg"
  alt="工具调用生命周期图，展示用户请求如何经过 LLM、tool_call、Runtime、tool_result 再回到 LLM 并产出最终答案。"
  width={1600}
  height={900}
/>
...
<Image
  src="/course-visuals/module-3/02-mcp-host-client-server-architecture.svg"
  alt="MCP Host、Client、Server 架构图，展示 Host 内的 MCP Client 通过 transport 连接多个 MCP Server，以及 Server 内部的 Tools、Resources、Prompts。"
  width={1600}
  height={900}
/>
...
<Image
  src="/course-visuals/module-3/03-tools-vs-skills-comparison-board.png"
  alt="Tools 与 Skills 对照图，比较原子操作和多步骤编排在粒度、状态、逻辑和示例上的差异。"
  width={1920}
  height={1080}
/>
...
- [ ] 每个工具都有清晰 description、严格参数 schema 和运行时校验
- [ ] 高风险操作默认最小权限、白名单、只读优先
- [ ] 单轮任务只暴露真正需要的工具，而不是把整个工具箱全丢给模型
- [ ] 把“原子 tool”和“多步 skill”拆成两层，而不是混成一个大工具
